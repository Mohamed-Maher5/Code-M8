...
# Prevents 300s waits caused by sending full file contents to the LLM
...
class Synthesizer:
    ...
    def synthesize(
        ...
        """
        1. Strip runner results
        2. Trim outputs to MAX_OUTPUT_CHARS so summarize() is fast
        3. Set status → orchestrator / writing answer
        4. Call orchestrator.summarize()
        5. Fallback to raw output if summarize fails
        """
        # Step 1 — exclude runner
        ...
        # Summarize only needs the key findings, not raw file dumps.
        trimmed = []
        for r in results:
            t = dict(r)
        ...
        # Step 3 — update icon to brain before the summarize LLM call
