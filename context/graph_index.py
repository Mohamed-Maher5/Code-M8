...
def query_context(question: str, k: int = 5) -> str:
    ...
    Returns:
        str: Context retrieved from graph
    """
    if not graph_rag_enabled():
        return "Graph RAG not configured."
    driver = get_driver()
    if driver is None:
        return "Could not connect to Neo4j."
    try:
...
def ensure_indexed(workspace_root: str) -> bool:
    ...
    try:
        ...
        if stored_hash == current_hash:
            # Workspace unchanged - skip indexing but do cleanup
            logger.info(f"Workspace hash unchanged: {current_hash[:8]}")
            auto_cleanup(workspace_root)  # Cleanup in background
            return True
        ...
        # Step 6: Cleanup orphaned data after indexing
